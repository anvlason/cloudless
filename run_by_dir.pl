...
foreach my $dir(glob("$ARGV[0]/LC8*"))
{
#    print ("cloud_mask_bqa_v1.1.py $dir\n");
#    system("./cloud_mask_bqa_v1.1.py $dir");
    my $prodname = basename($dir);
    if((-s "$dir/$prodname\_B2_TOA.TIF" > 0)&(-s "$dir/$prodname\_B3_TOA.TIF" > 0)&(-s "$dir/$prodname\_B4_TOA.TIF" > 0)){
        print "add to stack $dir\n";
        open(lst2,">>$ARGV[0]/stack/stack_b2.txt");
        print lst2 "$dir/$prodname\_B2_TOA.TIF\n";
        close(lst2);
        open(lst3,">>$ARGV[0]/stack/stack_b3.txt");
        print lst3 "$dir/$prodname\_B3_TOA.TIF\n";
        close(lst3);
        open(lst4,">>$ARGV[0]/stack/stack_b4.txt");
        print lst4 "$dir/$prodname\_B4_TOA.TIF\n";
        close(lst4);
    }
}
...
system("mp_runner.py $ARGV[0]/stack $ARGV[1] $ARGV[2]");
